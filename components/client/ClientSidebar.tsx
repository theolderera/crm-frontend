  ...
  return (
    <aside className="hidden md:flex flex-col w-64 h-screen fixed left-0 top-0 bg-white dark:bg-[#020617] text-slate-900 dark:text-slate-300 border-r border-slate-200 dark:border-slate-800/60 shadow-sm dark:shadow-xl z-50 font-sans">
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1 scrollbar-hide">
        ...
        >
          <Trophy size={18} />
          <span className="text-sm font-semibold">{t("dashboard.leaderboard")}</span>
        </button>
        ...
        >
          ...
          <span className="text-sm font-semibold">{t("dashboard.support")}</span>
        </button>

        <div className="flex items-center justify-between px-2 mb-4">
          <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">{t("dashboard.months")}</p>
          {isMentor && (
            <button
              onClick={onNewMonth}
